// This file was generated by Mendix Studio Pro.
//
// WARNING: Only the following code will be retained when actions are regenerated:
// - the import list
// - the code between BEGIN USER CODE and END USER CODE
// - the code between BEGIN EXTRA CODE and END EXTRA CODE
// Other code you write will be lost the next time you deploy the project.
// Special characters, e.g., é, ö, à, etc. are supported in comments.

package hashmap.actions;

import com.mendix.systemwideinterfaces.core.IContext;
import hashmap.implementation.MxHashMapHelper;
import com.mendix.systemwideinterfaces.core.IMendixObject;
import com.mendix.systemwideinterfaces.core.UserAction;

/**
 * Checks whether the map contains the given key
 */
public class JAVA_Hashmap_ContainsKey extends UserAction<java.lang.Boolean>
{
	/** @deprecated use Map.getMendixObject() instead. */
	@java.lang.Deprecated(forRemoval = true)
	private final IMendixObject __Map;
	private final hashmap.proxies.MxHashMap Map;
	private final java.lang.String Key;

	public JAVA_Hashmap_ContainsKey(
		IContext context,
		IMendixObject _map,
		java.lang.String _key
	)
	{
		super(context);
		this.__Map = _map;
		this.Map = _map == null ? null : hashmap.proxies.MxHashMap.initialize(getContext(), _map);
		this.Key = _key;
	}

	@java.lang.Override
	public java.lang.Boolean executeAction() throws Exception
	{
		// BEGIN USER CODE
		return MxHashMapHelper.getHashMap(this.Map.getMendixObject()).getMap().containsKey(this.Key);
		// END USER CODE
	}

	/**
	 * Returns a string representation of this action
	 * @return a string representation of this action
	 */
	@java.lang.Override
	public java.lang.String toString()
	{
		return "JAVA_Hashmap_ContainsKey";
	}

	// BEGIN EXTRA CODE
	// END EXTRA CODE
}
